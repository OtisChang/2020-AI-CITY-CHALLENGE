...
vis = visdom.Visdom(env='AI_city_reID_py36', port=8098)
...
class Main(object):
    ...
    def train(self):
        ...
        for batch, (inputs, id_label, color_label) in enumerate(self.train_loader):
            ...
            id_label = id_label.to('cuda', dtype=torch.int64)
            color_label = color_label.to('cuda', dtype=torch.int64)
            self.optimizer.zero_grad()
            outputs = self.model(inputs)
            #print(type(inputs), type(outputs), type(id_label), type(color_label))
    ...
    def evaluate_ai(self):
        # self.model.load_state_dict(torch.load('weights/pyramid_v1.pth'))
        # self.model.eval()
...
def main():
    ...
    pretrained_dict = torch.load('D:/environments/AI_city_reID_py36/AICity-track2-Re-id-csmvl310/weights/modelv5_100.pth')
